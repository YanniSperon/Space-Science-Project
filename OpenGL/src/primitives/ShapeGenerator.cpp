...
ShapeData ShapeGenerator::loadRectangle(glm::vec2 minExtent, glm::vec2 maxExtent, float z, glm::vec2 bottomLeftTexCoord, glm::vec2 topRightTexCoord)
{
	ShapeData ret;

	Vertex positions[] = {
		glm::vec3(minExtent.x, minExtent.y, z), // 0
		glm::vec3(1.0f, 0.0f, 0.0f), // color
		glm::vec2(bottomLeftTexCoord.x, bottomLeftTexCoord.y),        // texCoord

		glm::vec3(maxExtent.x, minExtent.y, z), // 1
		glm::vec3(0.0f, 1.0f, 0.0f), // color
		glm::vec2(topRightTexCoord.x, bottomLeftTexCoord.y),        // texCoord

		glm::vec3(maxExtent.x, maxExtent.y, z), // 2
		glm::vec3(0.0f, 0.0f, 1.0f), // color
		glm::vec2(topRightTexCoord.x, topRightTexCoord.y),        // texCoord

		glm::vec3(minExtent.x, maxExtent.y, z), // 3
		glm::vec3(1.0f, 1.0f, 1.0f), // color
